...
public class SimLockManagerPlugin extends SimLockManager implements AddonManager.InitialCallback {
    ...
    public void registerForSimLocked(final Context context) {
        ...
        mAddonContext.bindService(new Intent("com.android.sprd.telephony.server.RADIOINTERACTOR_SERVICE")
                ...
                public void onServiceConnected(ComponentName name, IBinder service) {
                    ...
                    for (int i = 0; i < phoneCount; i++) {
                        ...
                        mRadioInteractor.listen(mCallbackListener[i],
                                RadioInteractorCallbackListener.LISTEN_SIMLOCK_NOTIFY_EVENT | RadioInteractorCallbackListener.LISTEN_EXPIRE_SIM,
                                false);
            ...
            IntentFilter simStateIntentFilter = new IntentFilter();
            simStateIntentFilter.addAction(TelephonyIntents.ACTION_SIM_STATE_CHANGED);
            context.registerReceiver(mSimStateReceiver, simStateIntentFilter);
    }
    ...
    public void showPanel(Context context, Message msg) {
        ...
        case RadioInteractorCallbackListener.LISTEN_SIMLOCK_NOTIFY_EVENT:
            ...
            case IccCardStatusEx.SIM_STATE_NETWORKSUBSET_LOCKED:
                ...
                sim_network_subset_locked_count++;
            ...
            case IccCardStatusEx.SIM_STATE_SERVICEPROVIDER_LOCKED:
                ...
                sim_service_provider_locked_count++;
            ...
            case IccCardStatusEx.SIM_STATE_NETWORK_SUBSET_LOCKED_PUK:
                ...
                sim_network_subset_locked_puk_count++;
            ...
            case IccCardStatusEx.SIM_STATE_CORPORATE_LOCKED_PUK:
                ...
                sim_corporate_locked_puk_count++;
        ...
        if (mAddonContext.getResources().getBoolean(R.bool.ignore_sim_network_locked_events)) {
            ...
            if(Resources.getSystem().getBoolean(com.android.internal.R.bool.config_subsidyLock)){
                int attemptsCount = SimLockUtils.getSimLockUnLockTimes(type, phoneId);
                Log.i(TAG, "For subsidyLock simlock, just return the unlock times:" + attemptsCount);
                ndpPanel = new IccSimLockDepersonalizationPanelSprd(this, mAddonContext, phone, attemptsCount, type);
            } else {
                ndpPanel = new IccSimLockDepersonalizationPanelSprd(this, mAddonContext, phone, remainCount, type);
            }
            if (isPinOrPuk) {
    ...
    public void showPanelForUnlockByNv(Context context) {
        ...
        IccPanelSprd ndpPanel = null;
        ndpPanel = new IccSimLockDepersonalizationPanelSprd(this, mAddonContext, phone,
            ...
            if (action.equals(TelephonyIntents.ACTION_SIM_STATE_CHANGED)) {
                if (Resources.getSystem().getBoolean(
                    ...
                    if (IccCardConstants.INTENT_VALUE_ICC_ABSENT.equals(state)) {
                        ...
                        Log.d(TAG, "mSimAbsentFlag = " + mSimAbsentFlag);
                        if ((mSimAbsentFlag == ((1 << phoneCount) - 1))) {
                            Log.d(TAG, "showPanelForUnlockByNv because all sim absent");
                            showPanelForUnlockByNv(context);
                        }
                    }
                }
            }
    ...
    void resetLockedCount(int type){
        Log.d(TAG, "resetLockedCount type = " + type);
        switch(type) {
