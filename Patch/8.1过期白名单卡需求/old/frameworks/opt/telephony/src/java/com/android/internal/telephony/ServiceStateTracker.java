...
public class ServiceStateTracker extends Handler {
    ...
    */
        ...
        public void onSubscriptionsChanged() {
            ...
            if (mPreviousSubId.getAndSet(subId) != subId) {
                ...
                if (mSubscriptionController.getSlotIndex(subId)
                    ...
                    /* SPRD: modify by BUG 843418 @{ */
                    Message msg = obtainMessage(EVENT_SIM_NOT_INSERTED);
                    msg.arg1 = subId;
                    sendMessage(msg);
                    /* @} */
                }
    ...
    public ServiceStateTracker(GsmCdmaPhone phone, CommandsInterface ci) {
        ...
        log("init : airplane mode = " + airplaneMode + " enableCellularOnBoot = " +
                enableCellularOnBoot);
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case EVENT_SIM_NOT_INSERTED:
                ...
                /* SPRD: modify by BUG 843418 @{ */
                if (msg != null) {
                    cancelAllNotifications(msg.arg1);
                }
                /* @} */
                mMdn = null;
                mMin = null;
            ...
            case EVENT_ALL_DATA_DISCONNECTED:
                //bug831530 emergency call fail after airplane mode on
                if (SystemProperties.getBoolean("persist.radio.airplane_mode_on",false)) {
                    int dds = SubscriptionManager.getDefaultDataSubscriptionId();
                    ProxyController.getInstance().unregisterForAllDataDisconnected(dds, this);
                } else {
                    /*SPRD: for bug 599299 @{*/
                    mPhone.mDcTracker.unregisterForAllDataDisconnected(this);
                    /* @} */
                }
                synchronized(this) {
                    if (mPendingRadioPowerOffAfterDataOff) {
    ...
    protected void handlePollStateResult(int what, AsyncResult ar) {
        ...
        if (ar.exception != null) {
            ...
            if (err == CommandException.Error.RADIO_NOT_AVAILABLE) {
                ...
                /* SPRD: [bug713284] Make sure service state can be set as POWER_OFF without delay. @{ */
                if (mCi.getRadioState() == CommandsInterface.RadioState.RADIO_OFF
                        && mSS.getState() != ServiceState.STATE_POWER_OFF) {
                    pollState();
                    if (DBG) {
                        log("handlePollStateResult: set service state as POWER_OFF.");
                    }
                } else {
                    cancelPollState();
                }
        ...
        switch (what) {
            ...
            case EVENT_POLL_STATE_OPERATOR: {
                if (mPhone.isPhoneTypeGsm()) {
                    ...
                    // SPRD Add for Bug814455. Update operator name for Dual VoLTE DSDA:
                    if(!ImsManagerEx.isDualLteModem() && ImsManagerEx.isDualVoLTERegistered()
                        ...
                        String dataPhoneOperator = TelephonyManager.getTelephonyProperty(dataPhoneId, TelephonyProperties.PROPERTY_OPERATOR_ALPHA, "");
                        String dataPhoneOperatorNum = TelephonyManager.getTelephonyProperty(dataPhoneId, TelephonyProperties.PROPERTY_OPERATOR_NUMERIC, "");
                        if(!TextUtils.isEmpty(dataPhoneOperator) && !TextUtils.isEmpty(dataPhoneOperatorNum)
                                && !dataPhoneOperator.equals(mNewSS.getOperatorAlpha())) {
                            log("Update operator name for DSDA. Orig name = " + mNewSS.getOperatorAlpha()
                                    + " New name = " + dataPhoneOperator);
                            mNewSS.setOperatorName(dataPhoneOperator, dataPhoneOperator, dataPhoneOperatorNum);
                        }
                    }
                } else {
                    String opNames[] = (String[])ar.result;
            ...
            if (configLoader != null) {
                try {
                    ...
                    if (alwaysOnHomeNetwork(b)) {
                        ...
                        roaming = false;
                    /* SPRD: bug691317 Free Carrier Ignore domestic roaming. @{ */
                    } else if (isNonRoamingInDomesticNetwork(mNewSS.getOperatorNumeric())) {
                        log("updateRoamingState-GSM: carrier config override domestic non roaming");
                        roaming = false;
                    /* @} */
            ...
            if (configLoader != null) {
                try {
                    ...
                    /* SPRD: bug691317 Free Carrier Ignore domestic roaming. @{ */
                        ...
                        log("updateRoamingState-CDMA:" +
                                "carrier config override domestic non roaming");
                        setRoamingOff();
                    /* @} */
                    } else if (isNonRoamingInGsmNetwork(b, mNewSS.getOperatorNumeric())
    ...
    protected void updateSpnDisplay() {
        ...
        if (mPhone.isPhoneTypeGsm()) {
            ...
            if (combinedRegState == ServiceState.STATE_OUT_OF_SERVICE
                ...
                // SPRD: add by fix bug795900 to translation the display name
            ...
            //SPRD: [bug634744] Return original loaded from sim.
            //String spn = (iccRecords != null) ? iccRecords.getServiceProviderName() : "";
            String spn = ((TelephonyManager) mPhone.getContext().
                    getSystemService(Context.TELEPHONY_SERVICE)).getSimOperatorNameForPhone(mPhone.getPhoneId());
            //SPRD: add by fix bug795900 to translation the display name
            ...
            if (!TextUtils.isEmpty(spn) && !TextUtils.isEmpty(wfcVoiceSpnFormat) &&
                ...
                plmn = String.format(wfcVoiceSpnFormat, originalPlmn);
                    ...
                    //SPRD: Bug#727767 not case sensitive
                    || (showPlmn && plmn.equalsIgnoreCase(spn))) {
    ...
    private void pollStateDone() {
        ...
        // No need to update in L+L version.
        if(!ImsManagerEx.isDualLteModem() && ImsManagerEx.isDualVoLTERegistered()
            ...
            log("Dual VoLte Registered, update RAT and voice registration state.");
            mNewSS.setVoiceRegState(ServiceState.STATE_IN_SERVICE);
            mNewSS.setRilVoiceRadioTechnology(ServiceState.RIL_RADIO_TECHNOLOGY_LTE);
            mNewSS.setRilDataRadioTechnology(ServiceState.RIL_RADIO_TECHNOLOGY_LTE);
        }
        ...
        if (hasChanged) {
            ...
            if (isInvalidOperatorNumeric(operatorNumeric)) {
                ...
                if (hasVoiceRegStateChanged || hasDataRegStateChanged) {
                    mPhone.mDcTracker.setSinglePDNAllowedByNetwork(operatorNumeric);
                }
                /* @} */
                try {
                ...
                if (!mNitzUpdatedTime && !mcc.equals("000") && !TextUtils.isEmpty(iso)
                    ...
                    } /** SPRD Modified For Bug 687270 @{ **/
                    else if (uniqueZoneIds.size() > 1 && TextUtils.isEmpty(mSavedTimeZone)) {
                        for (int i = 0; i < mDefaultTZForMultiTZCountry.length; i++) {
                            if (iso.equals(mDefaultTZForMultiTZCountry[i][0])) {
                                setAndBroadcastNetworkSetTimeZone(
                                        mDefaultTZForMultiTZCountry[i][1]);
                                if (DBG) {
                                    log("pollStateDone: no nitz but get default time zone by "
                                            + "iso-cc=" + iso + " with zoneID="
                                            + mDefaultTZForMultiTZCountry[i][1]);
                                }
                                break;
                            }
    ...
    private void revertToNitzTimeZone() {
        ...
        if (mSavedTimeZone != null) {
            ...
            // SPRD Add for Bug724553
            // Android original design of TZ update by country code is triggered by service state change.
            // If service state didn't change, TZ cannot update.
    ...
    /* SPRD: modify by BUG 843418 @{ */
    private void cancelAllNotifications(int subId) {
        if (DBG) log("setNotification: cancelAllNotifications for subId : " + subId);
        NotificationManager notificationManager = (NotificationManager)
                mPhone.getContext().getSystemService(Context.NOTIFICATION_SERVICE);
    ...
    /*SPRD: bug831530 emergency call fail after airplane mode on @{*/
    public boolean isPowerOffAsap(DcTracker dcTracker,boolean isAirplaneMode) {
        int dds = SubscriptionManager.getDefaultDataSubscriptionId();
        if (!isAirplaneMode && dcTracker.isDisconnected()) {
            return true;
        } else if (isAirplaneMode && (dcTracker.isDisconnected()
                && (dds == mPhone.getSubId()
                || (dds != mPhone.getSubId()
        ...
        synchronized (this) {
            if (!mPendingRadioPowerOffAfterDataOff) {
                if (mPhone.isPhoneTypeGsm() || mPhone.isPhoneTypeCdmaLte()) {
                    boolean isAirplaneMode = SystemProperties.getBoolean("persist.radio.airplane_mode_on",false);
                    int dds = SubscriptionManager.getDefaultDataSubscriptionId();
                    // To minimize race conditions we call cleanUpAllConnections on
                    // both if else paths instead of before this isDisconnected test.
                    //SPRD: power off radio when dataconnection is disconnected on
                    // this phone, ignore the dataconnecton state on dds.
                    boolean isPowerOffAsap = isPowerOffAsap(dcTracker,isAirplaneMode);
                    if (isPowerOffAsap) {
                        ...
                        /*SPRD: bug831530 emergency call fail after airplane mode on @{*/
                        if (!isAirplaneMode) {
                            if (DBG) log("Data is active.  Wait for all data disconnect");
                            /*SPRD: for bug 599299 @{*/
                            // Data is not disconnected. Wait for the data disconnect complete
                            // before sending the RADIO_POWER off.
                            mPhone.mDcTracker.registerForAllDataDisconnected(this,
                                    EVENT_ALL_DATA_DISCONNECTED, null);
                            mPendingRadioPowerOffAfterDataOff = true;
                            /* @} */
                        } else if (dds != mPhone.getSubId()
                                && !ProxyController.getInstance().isDataDisconnected(dds)){
                            if (DBG) log("both sim need to wait for data disconnected then power off radio");
    ...
    /* SPRD: bug691317 Free Carrier Ignore domestic roaming for specific country. @{ */
    protected final boolean isNonRoamingInDomesticNetwork(String network) {
        return isNonDomesticRoamingEnabled() && inSameCountry(network);
    }

    protected final boolean isNonDomesticRoamingEnabled() {
        boolean retVal = mPhone.getContext().getResources().getBoolean(
                com.android.internal.R.bool.config_enableNonRoamingInDomesticNetwork);
        if (DBG) {
            log("isNonDomesticRoamingEnabled=" + retVal);
        }
        return retVal;
    }
    /* @} */
    ...
    /** SPRD: Add for Bug724553 */
    private void updateTimezoneByNetworkCountryCode() {
        try {
            String iso = ((TelephonyManager) mPhone.getContext().getSystemService(Context.TELEPHONY_SERVICE)).
                        getNetworkCountryIsoForPhone(mPhone.getPhoneId());
            if (!TextUtils.isEmpty(iso) && getAutoTimeZone()) {
                String networkZoneId = "";
                List<String> uniqueZones = TimeUtils.getTimeZoneIdsWithUniqueOffsets(iso);
                if(uniqueZones.size() == 1) {
                    networkZoneId = uniqueZones.get(0);
                } else if(uniqueZones.size() > 1 && TextUtils.isEmpty(mSavedTimeZone)) {
                    for (int i = 0; i < mDefaultTZForMultiTZCountry.length; i++) {
                        if (iso.equals(mDefaultTZForMultiTZCountry[i][0])) {
                            networkZoneId = mDefaultTZForMultiTZCountry[i][1];
                            break;
                        }
                    }
                }
                if (!TextUtils.isEmpty(networkZoneId)) {
                    setAndBroadcastNetworkSetTimeZone(networkZoneId);
                    if (DBG) log("updateTimezoneByNetworkCountryCode: no nitz but get default time zone" +
                                " by iso-cc=" + iso + " with zoneID=" + networkZoneId);
                } else {
                    loge("updateTimezoneByNetworkCountryCode: no nitz and cannot get default time zone" +
                            " by iso-cc=" + iso);
                }
            }
        } catch (Exception ex) {
            loge("updateTimezoneByNetworkCountryCode: countryCodeForMcc error" + ex);
        }
    }
    /** SPRD: Add for Bug877423 */
    private boolean isInEmergencyCall() {
        ...
        if(!fgCall.isIdle()) {
            for (Connection cn : fgCall.getConnections()) {
                if (PhoneNumberUtilsEx.isEmergecyNumber(mPhone.getPhoneId(),
                        cn.getAddress())) {
                    return true;
                }
            }
